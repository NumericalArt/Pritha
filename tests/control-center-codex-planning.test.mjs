...
test("Codex numeric settings preserve drafts until explicit validation at save", () => {
  ...
  assert.doesNotMatch(codexSettingsSource, /onBlur=\{commitMaxPlanStepsDraft\}/);
  assert.match(settingsRouteSource, /validateRuntimeNumbers\(body\)/);
});
