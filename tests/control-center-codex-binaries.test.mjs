...
test("generic CODEX_BIN cannot silently turn the primary App route into CLI", async () => {
  ...
  try {
    ...
    const options = { env: { CODEX_BIN: cli }, homeDir: "/Users/tester", existsSync: (candidate) => candidate === desktop };
    ...
    const desktopLegacy = { ...options, env: { CODEX_BIN: desktop }, existsSync: (candidate) => candidate === desktop || candidate === "/Users/tester/.local/bin/codex" };
    assert.equal(loaded.module.resolveCodexAppBinary(undefined, desktopLegacy), desktop);
    assert.equal(loaded.module.resolveCodexCliBinary(undefined, desktopLegacy), "/Users/tester/.local/bin/codex");
  } finally {
    loaded.cleanup();
  }
});
...
test("missing desktop App leaves the App route unavailable and preserves CLI fallback", async () => {
  const loaded = await loadBinaryModule();
  try {
    const options = { env: {}, homeDir: "/Users/tester", existsSync: () => false };
