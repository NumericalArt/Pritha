...
source_version: "Read-only ND source 8307bec20fb3b6a312d4478eafb50157c1603786; local stock CLI help 0.153.0; ND runtime advertises exec_resume and steerTurn=false"
...
история, задачи и службы в рамках написания инструкции не менялись. Материнские
изменения реализованы и проверены; их component pins приведены в конце документа,
а итоговый source pin и состояние выпуска — в release manifest. Для переноса
по-прежнему нужны локальные ND evidence. Выпуск матери не обновляет ND.

Расширение материнского плана T1/P8/P9 добавляет темы и выпуск обычных clones.
ND служит источником палитр и не входит в этот rollout. Обратный перенос
материнского Classic default или theme storage v2 в ND не подразумевается:
сохранять её принятые темы, собственный выбор и локальный контракт настроек.
...
## ND implementation handoff: общие модули и конкретные границы

Каноническое решение описано в
[execution coordination ADR](../05_decisions/2026-09-08-task-chat-execution-coordination.md),
а команды — в [operations](task-chat-concurrency-operations.md). Кандидат ещё должен
получить финальный source pin и per-instance release receipts; не брать mutable
рабочую ветку вместо проверенного release.

Переносить отдельным review следующие общие модули: `execution-coordinator`,
`execution-channel`, `execution-lifecycle`, `owned-process`, `task-workspace`,
`runtime-probe` из `scripts/lib/` и их `.d.mts`; затем adapter-specific calls.
Сохранить ND provider ledger, credentials path, CLI argv/config и provider model
selection. Не переносить stock `app-server.ts` или native RPC capabilities как
замену CLI adapter. Native `user_input`, permission grants и `turn/steer` остаются
выключенными, пока CLI/provider не предъявит отдельный проверенный контракт.

В ND original ChildProcess регистрируется в private IPC только для разрешённых
действий своего CLI transport. Stop использует реальный ChildProcess, process birth,
PGID и generation; после рестарта PID из JSON не авторизует kill. Секретные ответы
не кладутся в SQLite mailbox. До проверки provider concurrency использовать предел
1; затем синтетические 2/3 независимые задачи и подтверждённые provider limits.
Budget/rate limits проверяются совместно с общим локальным admission, без bypass
существующего ND ledger и без автоматического fallback на OpenAI.

Добавить protocol-1 build marker с точным BUILD_ID, drain guard и отдельную
активацию после strict health. Для primary/replica новый store создаётся paused.
До активации проверить миграцию legacy guards на копии ND state. Если ND уже имеет
свой transactional authority, составить однозначный mapping вместо второго
независимого writer. Unknown provider attempt остаётся charged/blocked до provider
receipt reconciliation. Старую ND сборку без понимания нового writer нельзя
использовать как rollback после появления новых intents.

Обязательные ND тесты сверх общих fixtures: provider refusal/429, потеря CLI ack,
завершение основного процесса при живом descendant, сохранение ND usage receipts,
перезапуск до/после dispatch, shared subject Voice ownership, два ChildProcess
с адресным Stop A при работающем B, query прежней истории без resume, invalid model
без замены поставщика. Документ не означает внедрения этих изменений в ND.

Материнские source pins для первого переноса: foundation `f8331960fc01149adc83ff088b8dcd5777e5f2ec`, workspace selector
`1da4ea1fd8d2af1e1f4c4804d2264769dd919c88`. Изменение default нового UI не отменяет ND-specific provider и
CLI sandbox policy: перенести явный selector с captured payload, capability проверять
локально. Полный candidate tag и SHA берутся из release handoff manifest.
