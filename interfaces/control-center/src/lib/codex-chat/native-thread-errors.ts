...
export function classifyNativeThreadReadFailure(value: unknown): NativeThreadReadFailure {
  ...
  if (/unsupported.*(format|version)|invalid.*(rollout|schema)|failed to (parse|deserialize)|history_format_unsupported/i.test(message)) return "history_format_unsupported";
