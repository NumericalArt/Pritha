...
function syntheticCodexTaskPlan(task: Record<string, unknown>, source: CodexTaskPlan["source"] = "synthetic"): CodexTaskPlan {
  ...
  if (taskType === "agent_creation") {
    return {
      ...
      reason: "Agent creation requires separately approved architecture and outcome, research-gated scaffold, revision-bound delivery Trials and card-first Control Center registration.",
      riskLevel: "high",
      requiresOperatorInput: false,
      operatorQuestions: [],
      steps: [
        {
          id: "contract_outcome_research_gate",
          title: "Contract, Outcome And Research Gate",
          goal: "Create or validate the accepted agent contract, separately approve and compile the locked Outcome Spec, run Pritha memory/pattern/current-source research, and confirm the scaffold gate is complete or explicitly not-applicable.",
          expectedOutput: "Accepted contract, approved Outcome Spec with Trial plan, and research gate evidence or a typed blocker.",
          needsWrite: true,
        ...
        {
          id: "deliver_outcome",
          title: "Build And Verify Outcome",
          goal: "Run Pritha outcome delivery in a disposable Git worktree until all locked Trials pass for a committed revision or a typed blocker needs the operator.",
          expectedOutput: "Revision-bound delivery report in verified/awaiting_acceptance state, or a typed blocker with one actionable question.",
          needsWrite: true,
          needsNetwork: false,
          operatorGate: false,
        },
        ...
        },
        {
          ...
          goal: "Summarize outcome verification, acceptance state and whether the card is ready, blocked or missing, then list the next operator-visible task.",
          expectedOutput: "Concise completion summary; creation is not complete without verified outcome evidence or a typed blocker, and a visible card.",
          needsWrite: false,
          needsNetwork: false,
          operatorGate: false,
        },
...
function buildPlanningTask(task: Record<string, unknown>) {
  return {
    ...
    task: [
      ...
      "For agent_creation, the plan must include contract_outcome_research_gate, deliver_outcome and verify_control_center_card steps. It must not treat scaffold as completion, conflate machine verification with user acceptance, or finish while card readiness is missing.",
      "",
      "Original task:",
