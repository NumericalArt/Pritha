...
for (const width of [1440, 390]) test(`composer working indicator preserves typing and follows the selected task at ${width}px`,async({page},testInfo)=>{
  ...
  const fieldBounds=await input.boundingBox(), indicatorBounds=await indicator.boundingBox();
  ...
  const appearance=await input.evaluate(el=>({size:getComputedStyle(el).fontSize,color:getComputedStyle(el,'::placeholder').color}));
  expect(await indicator.evaluate(el=>({size:getComputedStyle(el).fontSize,color:getComputedStyle(el).color}))).toEqual(appearance);
  await page.mouse.click(fieldBounds!.x+20,fieldBounds!.y+10); await expect(input).toBeFocused();
  await page.screenshot({path:testInfo.outputPath('working-placeholder.png'),fullPage:true});
  await input.fill('Keep this next task draft');
  await expect(indicator).toHaveCSS('opacity','0');
  await expect(composer.getByRole('button',{name:'Send',exact:true})).toBeDisabled();
  await input.press('Enter'); expect(f.requests).toHaveLength(0);
